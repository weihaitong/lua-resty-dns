...
local function _build_request(qname, id, no_recurse, opts)
    ...
    local zero = "\0"
    local opt = "\0" .. char(0x29)
    local payload = char(0x10) .. "\0"
    local datalen = "\0" .. char(12)
    local optcode = "\0" .. char(8)
    local optlen = "\0" .. char(8)
    local family = "\0\1"  -- ipv4
    local srcmsk = char(32)
    local scpmsk = "\0"
    ...
    return {
        ...
        name, typ, class, zero, opt, payload, zero, zero, 
        zero, zero, datalen, optcode, optlen, family, 
        srcmsk, scpmsk, char(tonumber(ip_part[1])), char(tonumber(ip_part[2])), 
        char(tonumber(ip_part[3])), char(tonumber(ip_part[4]))
    }
end
